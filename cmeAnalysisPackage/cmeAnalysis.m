...
ip.addParamValue('condDir', []);
ip.addParamValue('chNames', []);
ip.addParamValue('markers', []);
...
if isempty(data)
    ...
    if isempty(ip.Results.condDir) || isempty(ip.Results.chNames) ||  isempty(ip.Results.markers)
        data = loadConditionData('Parameters', parameters);
    else
        data = loadConditionData('Parameters', parameters, 'condDir', ip.Results.condDir, 'chNames', ip.Results.chNames, 'markers', ip.Results.markers);
    end
end
...
% 1) Detection
%-------------------------------------------------------------------------------
if isempty(ip.Results.Master)
    runDetection(data, 'SigmaSource', ip.Results.GaussianPSF, opts{:});
else
    runDetection(data, 'SigmaSource', ip.Results.GaussianPSF, 'Master', ip.Results.Master, opts{:});
end
