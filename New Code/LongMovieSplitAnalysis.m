function LongMovieSplitAnalysis(movie,MaxSectionSize,newmovie,framegap,Threshs)
...
% cmeanalysis_auto;
% p=gcp;
% delete(p) %Shut down parallel pool because if it shuts down later it can cause errors
% cd(wd)
...
close(h)
TraceFinalization(Threshfxyc,Thresh,movie,4,1,0,.75);
