function fxyc=RedoEndDetection(fxyc)
...
frames=max(max(fxyc(:,1,:))); %may cause problems if nothing is found on last frame
% h=waitbar(0,'Finalizing Traces');
for i=1:length(fxyc(1,1,:))
%     waitbar(i/length(fxyc(1,1,:)))
    Ints=nonzeros(fxyc(:,5,i));
    if ~isempty(Ints)
        ...
        ends=0;
        if ~isempty(BegPL) && isempty(find(fxyc(:,1,i)==1)) && (min(nonzeros(PreInts))<=max(Ints)*startfade || min(nonzeros(PreInts))<=background+backgroundSD)
            ends=ends+1;
            
        end
        if ~isempty(FinPL) && isempty(find(fxyc(:,1,i)==frames)) && (min(nonzeros(PostInts))<=max(Ints)*endfade || min(nonzeros(PostInts))<=background+backgroundSD)
            ends=ends+2;
            
        end
        if ~isempty(find(fxyc(:,1,i)==frames))
            ends=ends+8;
        end
        if ~isempty(find(fxyc(:,1,i)==1))
            ends=ends+4;
        end
        c=0;
        if ends==0 
