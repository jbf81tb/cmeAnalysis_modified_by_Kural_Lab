function [fxyc,num]=TrackConnector(fxyc,Thresh,dThresh2,aThresh,fThresh) %aThresh no longer incorporated after it caused an unfortunate trace split--cmeAnalysis amplitude may be dependent on the past/future of the trace
...
for i=1:B %Find any traces that are start/end close enough in position, time, and intensity
    waitbar(i/B)
    for j=1:B
       if i~=j && startfxyi(i,1)~=0 && startfxyi(j,1)~=0 && startfxyi(i,1)-finishfxyi(j,1)<=fThresh && startfxyi(i,1)-finishfxyi(j,1)>0 && sqrt((startfxyi(i,2)-finishfxyi(j,2))^2+(startfxyi(i,3)-finishfxyi(j,3))^2)<=dThresh2
           clear tempfxyc
           num=num+1;
