function LongMovieThreshSelectionWrapper
%Navigate to the folder with all the movies you want to find threshs for
wd=pwd;
folderlist=ls;
dirname = cell(size(folderlist,1)-2);
...
for i = 1:size(dirname,2)
    ...
    if movies
        for i2 = 1:size(dirname2,2)
            ...
            file=moviename{i2};
            if i2==1
                SectionSize{i} = length(imfinfo(strcat('\ch1\',file)));
            end
            FrameGap=str2double(dirname2{i2}(end-1));
            cd(wd2)
        end
        
        [Threshs{i}]=LongMovieThreshholdSelection(paths,moviename,SectionSize{i});
