...
for i=1:sections-1 %Go through all adjacent pairs of sections looking for corresponding trace ends
    ...
    for i2=1:B3
        ...
        if found==0 %No corresponding pit was found
            for i3=1:A1
                for i4=1:A2
                    Threshfxyc(i3,i4,index)=fxyc2(i3,i4,i2);
            ...
            free=find(Threshfxyc(:,1,routingold(f))==0,1,'first'); %Where new trace data can be put
            if isempty(free)
                [C1,~,~]=size(Threshfxyc);
                free=C1+1;
            end
            for i3=1:A1
                for i4=1:A2
                    Threshfxyc(free(1),i4,routingold(f))=fxyc2(i3,i4,i2);
                    
                end
                free(1)=free(1)+1;
            end
            routingnew(i2)=routingold(f);
