function ColorSelected(fxyc,Selected,MeanInt,file,newfile) %adds colored spots to tif stack (named file--rewrites it in a file named newfile) at desired locations--fzzzmmmc is a nzzz4 matrizzz with the n spots--column 1 frame,column 2 zzz position, column 3 mmm position, column 4 color (1 for green, 2 for red)
...
for i=1:frames
    ...
    for j=1:length(inds)
        if ismember(inds(j),Selected)
            index=find(Selected==inds(j));
            Mean=MeanInt(index);
            RelativeMean=(Mean-MinMean)/(MaxMean-MinMean);
            x=fxyc(1,2,inds(j));
            y=fxyc(1,3,inds(j));
            c=fxyc(1,4,inds(j));
            if x~=0 && y~=0
...
%         B(y,x,1)=3000*(RelativeMean);
%         B(y,x,2)=0;
%         B(y,x,3)=3000*(1-RelativeMean);
